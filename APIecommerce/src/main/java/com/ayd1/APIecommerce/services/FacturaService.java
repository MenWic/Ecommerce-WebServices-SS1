...
import com.ayd1.APIecommerce.services.pasarelas.PasarelaService;
import com.ayd1.APIecommerce.services.pasarelas.PasarelaServiceFactory;
...
public class FacturaService extends Reporte {
    ...
    @Autowired
    private PasarelaServiceFactory bancoServiceFactory;
    ...
    public byte[] guardarVenta(VentaRequest ventaRequest) throws Exception {
        ...
        //EMPEZAMOS
        PasarelaService pasarelaService = bancoServiceFactory.getPasarelaService(ventaRequest.getPasarela());
        String token = pasarelaService.geToken("ABCD", "1234"); //AQUI PEDIMOS TOKEN PARA HEADER EN PETICIONES (PASARELA A)=
        Usuario usuario = this.usuarioService.getUsuarioUseJwt(); //SE AGREGO A "UsuarioService"
        pasarelaService.emailExiste(token, usuario.getEmail());
        pasarelaService.realizarPago(token, usuario.getEmail(), total.toString(), ventaRequest.getTipo_transaccion());
        //Hasta aqui
